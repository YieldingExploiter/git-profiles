  ...
  if (conf.sshKey && process.platform !== 'win32') {
    try {
      ...
      const insecureStorageWarn = ()=>{
        ...
        console.log('');
        console.log('      export PROFILE_HELPER_KEY=$(tr -dc A-Za-z0-9 < /dev/urandom | head -c 4096 | xargs)');
        console.log('      export PROFILE_HELPER_IV=$(tr -dc A-Za-z0-9 < /dev/urandom | head -c 4096 | xargs)');
        console.log('');
